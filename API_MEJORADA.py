...
# 🔔 IMPORTAR CONTROLADOR DE NOTIFICACIONES
try:
    ...
    print("⚠️ notifications_controller.py no encontrado - Notificaciones deshabilitadas")
...
# ============================================================
notifications_controller = None
if FIREBASE_AVAILABLE and NOTIFICATIONS_AVAILABLE:
    try:
        notifications_controller = NotificationsController(db_instance=db)
        print("✅ Controlador de notificaciones inicializado correctamente")
    except Exception as e:
        print(f"⚠️ Error inicializando controlador de notificaciones: {e}")
        NOTIFICATIONS_AVAILABLE = False
...
def send_custom_notification():
    ...
    """
    if not NOTIFICATIONS_AVAILABLE or not notifications_controller:
        return jsonify({'error': 'Notificaciones no disponibles'}), 503
    ...
    try:
        ...
        if not usuario_id or not titulo or not cuerpo:
            return jsonify({
                'error': 'Faltan parámetros requeridos',
                'requeridos': ['usuario_id', 'titulo', 'cuerpo']
            }), 400
        
        # Convertir tipo a enum
        try:
            tipo = NotificationType[tipo_str.upper()]
        except KeyError:
            tipo = NotificationType.GENERAL
        
        # Enviar
        resultado = notifications_controller.enviar_notificacion(
            usuario_id=usuario_id,
            titulo=titulo,
            cuerpo=cuerpo,
            tipo=tipo,
        ...
        status_code = 200 if resultado.exitoso else 400
...
def send_gasto_notification():
    ...
    try:
        ...
        if not all([usuario_id, monto, categoria]):
            return jsonify({'error': 'Faltan parámetros: usuario_id, monto, categoria'}), 400
...
def send_budget_alert_notifications():
    ...
    try:
        ...
        usuario_id = data.get('usuario_id')
        categoria = data.get('categoria')
        gastado = data.get('gastado')
        presupuesto = data.get('presupuesto')
        
        if not all([usuario_id, categoria, gastado, presupuesto]):
            return jsonify({'error': 'Faltan parámetros requeridos'}), 400
        
        resultado = notifications_controller.enviar_alerta_presupuesto(
            usuario_id=usuario_id,
            categoria=categoria,
...
def send_ml_recommendation():
    ...
    try:
        ...
        confianza = data.get('confianza', 0.85)
        accion = data.get('accion', 'revisar')
        
        if not usuario_id or not recomendacion:
            return jsonify({'error': 'Faltan parámetros: usuario_id, recomendacion'}), 400
        
        resultado = notifications_controller.enviar_recomendacion_ml(
            usuario_id=usuario_id,
...
def send_anomaly_alert():
    ...
    try:
        ...
        usuario_id = data.get('usuario_id')
        tipo_anomalia = data.get('tipo_anomalia')
        monto = data.get('monto')
        descripcion = data.get('descripcion', '')
        categoria = data.get('categoria', 'general')
        
        if not all([usuario_id, tipo_anomalia, monto]):
            return jsonify({'error': 'Faltan parámetros requeridos'}), 400
        
        resultado = notifications_controller.enviar_alerta_anomalia(
            usuario_id=usuario_id,
...
@app.route('/api/notificaciones/tip', methods=['POST'])
@token_required
def send_financial_tip():
    """
    ...
    try:
        ...
        tip = data.get('tip')
        categoria = data.get('categoria', 'general')
        fuente = data.get('fuente', 'ML')
        
        if not usuario_id or not tip:
            return jsonify({'error': 'Faltan parámetros: usuario_id, tip'}), 400
        
        resultado = notifications_controller.enviar_tip_financiero(
            usuario_id=usuario_id,
            tip=tip,
            categoria=categoria,
            fuente=fuente
...
def send_batch_notifications():
    ...
    try:
        ...
        if not notificaciones:
            return jsonify({'error': 'Campo requerido: notificaciones'}), 400
        
        resumen = notifications_controller.enviar_lote(notificaciones)
...
def get_notification_history(usuario_id):
    ...
    try:
        ...
        historial = notifications_controller.obtener_historial(usuario_id, limite=limite)
...
def get_notification_stats(usuario_id):
    ...
    try:
        stats = notifications_controller.obtener_estadisticas(usuario_id)
